...
const areas = [
  ["Customer Scheduling", 91, "green"], ["Invoicing", 84, "green"], ["Customer Support", 63, "blue"], ["Purchasing", 34, "amber"], ["Payroll", 12, "red"],
] as const;

export function Hero() {
  const { openEarlyAccess } = useEarlyAccess();
  return (
    <section id="top" className="relative overflow-hidden pb-24 pt-[138px] sm:pb-28 sm:pt-[154px]">
      ...
      <div className="container-shell relative grid items-center gap-16 lg:grid-cols-[.93fr_1.07fr] lg:gap-14">
        ...
        <div className="relative lg:translate-x-3">
          ...
          <div className="absolute -bottom-7 -left-5 hidden rounded-xl border border-[#dfe5ec] bg-white p-3 shadow-[0_15px_35px_rgba(25,40,68,.13)] sm:flex sm:items-center sm:gap-3"><span className="grid size-8 place-items-center rounded-lg bg-[#eaf7f2] text-[#1b8b69]"><TrendingUp size={15} /></span><div><p className="text-[10px] text-[#7b8696]">Interruptions avoided</p><p className="text-sm font-semibold">72% this month</p></div></div>
        </div>
      </div>
    </section>
  );
